...
estimation_exp_model <- function(experiment_info){
  ...
  bestfit_df <- data.frame(
    ...
    theta1 = bestfit["theta1"], theta2 = exp(bestfit["theta2"]), 
    theta3 = exp(bestfit["sigma"]))
...
for (j in 1:length(result_list)){
  names(result_list)[j] <- paste(result_list[[j]]$bestfit_df[1, c("sample", "gene")], collapse = "_")
}
...
estimation_exp_model <- function(experiment_info){
  ...
  if (nrow(data_raw) < 3){
    ...
    # Prepare output
    bestfit_df <- data.frame(
      sample = experiment_info$sample, gene = experiment_info$gene, sgRNA = experiment_info$sgRNA,
      mllik = obj_value, theta1 = bestfit["theta1"], theta2 = exp(bestfit["theta2"]), 
      theta3 = exp(bestfit["sigma"]), t_14 = t_14, t_end = t_end)
...
for (j in 1:length(exp_model_results)){
  names(exp_model_results)[j] <- paste(
    exp_model_results[[j]]$bestfit_df[1, c("sample", "gene", "sgRNA")], collapse = "_")
}
...
estimation_exp_model <- function(experiment_info){
  ...
  bestfit_df <- data.frame(
    sample = experiment_info$sample, gene = experiment_info$gene, sgRNA = experiment_info$sgRNA,
    mouse = experiment_info$mouse, t_end = max(data_raw$time), mllik = obj_value, 
    theta1 = bestfit["theta1"], theta2 = exp(bestfit["theta2"]), 
    theta3 = exp(bestfit["sigma"]))
...
for (j in 1:length(exp_model_results)){
  names(exp_model_results)[j] <- paste(
    exp_model_results[[j]]$bestfit_df[1, c("sample", "gene", "sgRNA", "mouse")], collapse = "_")
}
