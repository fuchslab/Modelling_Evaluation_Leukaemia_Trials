...
gillespie_sim_ko_exp <- function(theta, x0, sigma, smat, props, design){
  
  # Empty data frame for simulation results per mouse and time point: 
  # values for x1, x2 and the observable, here called "value"
  data <- as.data.frame(matrix(NA, nrow = 0, ncol = 4))
  column_names <- c("name", "mouse", "time", "value")
  ...
  for(d in 1:nrow(design)){
    for (t in 1:design[d, "num_pdx"]) {
      ...
      df <- data.frame(rep("y", 2), rep(pdx_counter, 2), out_obs[, 1], obs)
  ...
  data[,"value"] <- data[,"value"] + rnorm(n = nrow(data), mean = 0, sd = sigma)
...
# confidence threshold more than twice, the CI boundaries are interpolated
